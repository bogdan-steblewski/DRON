...
class Macierz {
    ...
    SWektor<Typ, Siz> Tab[Siz];

public:
    Macierz(); 
    SWektor<Typ, Siz> operator[] (int i) const { return Tab[i]; }
    SWektor<Typ, Siz>& operator[] (int i) { return Tab[i]; }

    Typ Wyznacznik() const;
    void Transpozycja(Macierz& Mc);


    SWektor<Typ, Siz>  operator * (const SWektor<Typ, Siz>& W);
...
template<typename Typ, int Siz >
SWektor<Typ, Siz>  Macierz<Typ, Siz>::operator * (const SWektor<Typ, Siz>& W)
{
   
    SWektor<Typ, Siz> wynik;
    for (int i = 0; i < Siz; i++)
        wynik[i] = Tab[i] * W;
